...
class MediaListItemRenderVisitor: ListItemRenderVisitor {

   override fun visit(
      media: MalAnimeDetails.MalRelatedAnime,
      callbacksBundle: CallbacksBundle
   ): MalRelatedItemRender = MalRelatedItemRender(media)

   override fun visit(
      media: MalListGridItem,
      callbacksBundle: CallbacksBundle
   ): MalAnimeSearchFrameRender = MalAnimeSearchFrameRender(media)

   override fun visit(
      media: MalRankingListItem,
      callbacksBundle: CallbacksBundle
   ): MalRankingFrameRender = MalRankingFrameRender(media)

   override fun visit(
      media: MalSeasonalListItem,
      callbacksBundle: CallbacksBundle
   ): MalSeasonalAnimeRender = MalSeasonalAnimeRender(media)

   override fun visit(
      media: MalUserListItem,
      callbacksBundle: CallbacksBundle
   ): MalUserListAnimeRender = MalUserListAnimeRender(media)
}